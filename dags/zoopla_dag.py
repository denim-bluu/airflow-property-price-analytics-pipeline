...
def zoopla_etl_dag():
    ...
    def copy_to_redshift(s3_file_path, **kwargs) -> bool:
        ...
        iam_role = config["AWS"]["REDSHIFT_IAM_ROLE"]
        create_table = """
        ...
        logging.info(f"Creating table {constants.PROD_TABLE}")
        logging.info(f"Query: {create_table.format(table_name=constants.PROD_TABLE)}")
        run_redshift_statement(create_table.format(table_name=constants.PROD_TABLE))
        logging.info(f"Creating table {constants.PROD_TABLE}")
        logging.info(f"Query: {create_table.format(table_name=constants.STAGING_TABLE)}")
        run_redshift_statement(create_table.format(table_name=constants.STAGING_TABLE))
        logging.info(f"Copying data to {constants.STAGING_TABLE}")
        logging.info(f"Query: {sql_copy_to_stg}")
    ...
    def upsert_to_prod(**kwargs) -> bool:
        ...
        drop_stg_table = f"DROP TABLE {constants.STAGING_TABLE};"
        logging.info(f"Upserting data to {constants.PROD_TABLE}")
        logging.info(f"Deleting from {constants.PROD_TABLE}")
        logging.info(f"Query: {delete_from_table}")
        run_redshift_statement(delete_from_table)
        logging.info(f"Inserting into {constants.PROD_TABLE}")
        logging.info(f"Query: {insert_into_table}")
        run_redshift_statement(insert_into_table)
        logging.info(f"Dropping {constants.STAGING_TABLE}")
        logging.info(f"Query: {drop_stg_table}")
